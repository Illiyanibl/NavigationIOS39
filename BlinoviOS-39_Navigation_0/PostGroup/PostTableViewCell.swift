...
class PostTableViewCell: UITableViewCell {
    ...
    private func setupGesture(){
        let doubleTapCell = UITapGestureRecognizer(target: self, action: #selector(doubleTapCell))
        doubleTapCell.numberOfTapsRequired = 2
        contentView.addGestureRecognizer(doubleTapCell)
    }
    
    @objc func doubleTapCell(){
        doubleTapAction?()
        let image = imageCell.image
        imageCell.image = UIImage(named: "write")
